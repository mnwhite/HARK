...
def solveHealthInvestment(solution_next,CRRA,DiscFac,MedCurve,IncomeNext,IncomeNow,Rfree,Cfloor,LifeUtility,
    ...
    SubsidyFunc : function
        Function that yields health investment subsidy as a function of health.
...
#    dvdhNext_tiled = np.tile(dvdhNext,(1,Hcount,1))
    ...
    #iZero = np.logical_or(MargValueRatioAdj == 0.,np.isinf(MargValueRatioAdj)) # Optimal solution is i=0 for sure
    while (points_left > 0) and (LoopCount < MaxLoops):
        ...
        iGuess[Ratio == 0.] = 0.0
        #temp_bad = np.logical_and(iZero,these)
        #iGuess[temp_bad] = 0.0 # Set i=0 for points where investment is "bad"
    ...
    iCostArrayBig = np.maximum(MedPrice*CopayArrayBig*iLvlArrayBig - SubsidyFunc(hLvlArrayBig), 0.0)
    ...
    while (points_left > 0) and (LoopCount < MaxLoops):
        ...
        iGuess[temp] = np.maximum(iGuess[temp],SubsidyFunc(hGuess[temp])/MedPrice)
        hGuessNew = ExpHealthNextInvFunc(H - HealthProdFunc(iGuess))
...
class HealthInvestmentConsumerType(IndShockConsumerType):
    ...
    def getPostStates(self):
        ...
        '''
        t = self.t_sim
        iCost = np.maximum(self.CopayNow*self.MedPrice[t]*self.iLvlNow - self.SubsidyFunc(self.hLvlNow), 0.0)
        aLvlNow = self.bLvlNow - self.PremiumNow - self.xLvlNow - iCost
