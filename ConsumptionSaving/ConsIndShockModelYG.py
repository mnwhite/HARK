...
class ConsumerSolution(Solution):
    ...
    def __init__(self, cFunc=None, 
                       ...
                       mNrmMin=None, hNrm=None, MPCmin=None, MPCmax=None, vFunc=None):
        '''
...
#        self.vFunc   = ValueFunc(vFuncNvrs,self.CRRA)
    ...
    def makePFcFunc2(self):
        ...
        # Add two attributes to enable calculation of steady state market resources
        self.ExIncNext = 1.0 # Perfect foresight income of 1
        #until here there is nothing changed from before, the next one is the homework
        Mnow=self.solution_next.cFunc.x_list
        Cnow=self.solution_next.cFunc.y_list
        M,C,A=self.question1(Mnow,Cnow)
        M,C=self.question2(M,C)
        #question3
        if M[0]<self.BoroCnstArt:
            #question4, the intersection
            m_t_hat,c_hat=self.findIntersection([self.BoroCnstArt,0,0,-self.BoroCnstArt],np.array([M,C]))
            #question5+question6
            M,C=self.throwNodesBeforeMhat(m_t_hat,M,C)
        #question7
        M,C=self.checkNodesNumber(M,C)
        #question8
        cFuncNow=LinearInterp(M,C)
        #question9
        mNormMinNow,MPCMax=self.question9(M,C)
        # Construct the consumption function
        self.cFunc=cFuncNow
        self.MPCMax=MPCMax
        self.mNrmMinNow=mNormMinNow
       
    def question1(self,Mnow,Cnow,rho=1,Gamma_tp1=1):
        '''
        Q1
        ...
        ----------
        Mnow : array
            Wealth of previous nodes
        Cnow : array
            Consumption of previous nodes
        
        Returns
        -------
        M_nowArray : array
            Wealth of actual nodes without the last one
        C_nowArray : array
            Consumption of actual nodes without the last one
        A_nowArray : array
            Assets of actual nodes without the last one
        '''
        N=len(Mnow)
        M_nowArray = []
        C_nowArray = []
        ...
        for j in range(N-1):
            A_now=(Gamma_tp1/self.Rfree)*(Mnow[j]-1)
            C_now=(self.DiscFacEff*self.LivPrb*self.Rfree)**(-1/rho)*Gamma_tp1*Cnow[j]
            M_nowArray.append(C_now+A_now)
            C_nowArray.append(C_now)
            A_nowArray.append(A_now)
        return M_nowArray,C_nowArray,A_nowArray
    
    def question2(self,M_nowArray,C_nowArray):
        '''
        Q2
        ...
        M_nowArray : array
            Wealth of actual nodes with the last one
        C_nowArray : array
            Consumption of actual nodes with the last one
        '''
        kt=self.MPC
        ht=self.hNrmNow
        C_N=C_nowArray[-1]+kt
        M_N=M_nowArray[-1]+ht
        C_nowArray.append(C_N)
        M_nowArray.append(M_N)
        return M_nowArray,C_nowArray
    
    def findIntersection(self,line,interpolationPoints):
        '''
        Q4: find the intersection point of the interpolation and the borrowing constraint line
        
        Parameters
        ----------
        line : array
            the line defining the borrowing constraint, two points defining this line [x1,y1,x2,y2]
        interpolationPoints : array
            Interpolation of several points defined by [[x1,x2,...],[y1,y2,...]]
        
        Returns
        -------
        m_t_hat : float
            Wealth in the intersection point of the interpolation and the borrowing constraint line
        c_hat : float
            the consumption in m_t_hat
        '''
        aLine=(line[3]-line[1])/(line[2]-line[0])
        bLine=line[3]-aLine*line[2]
        j=0
        lastLineIndex=np.shape(interpolationPoints)[1]-2
        found=False
        while j<=(np.shape(interpolationPoints)[1]-2):
            aInterp=(interpolationPoints[1,j+1]-interpolationPoints[1,j+0])/(interpolationPoints[0,j+1]-interpolationPoints[0,j+0])
            bInterp=interpolationPoints[1,j+1]-aInterp*interpolationPoints[0,j+1]
            xi=(bInterp-bLine)/(aLine-aInterp)
            yi=aLine*xi+bLine
            if (xi<=np.max([interpolationPoints[0,j],interpolationPoints[0,j+1]]) and xi>=np.min([interpolationPoints[0,j],interpolationPoints[0,j+1]]) and yi<=np.max([interpolationPoints[1,j],interpolationPoints[1,j+1]]) and yi>=np.min([interpolationPoints[1,j],interpolationPoints[1,j+1]])):
                m_t_hat=xi
                c_hat=yi
                found=True
            if (j==lastLineIndex and found==False):
                m_t_hat=xi
                c_hat=yi
            j=j+1
        return m_t_hat,c_hat
    
    def throwNodesBeforeMhat(self,m_t_hat,M,C):
        '''
        Q5+Q6: adjust the part before the intersection point of the interpolation and the borrowing constraint line
        ...
        -------
        M : array
            Actual wealth with modified nodes, the adjusted nodes are those before the intersection
        C : array
            Actual consumption with modified first nodes
        '''
        M_new=[]
        C_new=[]
        M_new.append(self.BoroCnstArt)
        M_new.append(m_t_hat)
        C_new.append(0)
        C_new.append(m_t_hat-self.BoroCnstArt)
        for i in range(0,np.shape(M)[0]-1):
            if (M[i]>=m_t_hat):
                M_new.append(M[i])
                C_new.append(C[i])
        if (len(M_new)==2):
            M_new.append(m_t_hat+1)
            C_new.append(m_t_hat-self.BoroCnstArt+self.MPC)
        return M_new,C_new
    
    def checkNodesNumber(self,M,C):
        '''
        Q7: check if the number of nodes doesn't exeed a certain number, this number is defined withe the 
        instance of the PerfForesightConsumerType class: aXtraCount. In that case, nodes near the top are
        deleted
        ...
        -------
        M : array
            Actual wealth with modified nodes
        C : array
            Actual consumption with modified nodes
        '''
        
        MPC=(C[-1]-C[-2])/(M[-1]-M[-2])
        while (len(M)>self.aXtraCount):
            del M[-2]
            del C[-2]
            C[-2]=MPC*(M[-2]-M[-1])+C[-1]
        return M,C
    
    def question9(self,M,C):
        '''
        Q9: Get the smallest node and return the minimum wealth and the maximum marginal propensity to consume
        ...
        -------
        mNormMinNow : float
            Normalized wealth value in the smallest node
        MPCMax : float
            maximum marginal propensity to consume calculated in the smallest node
        '''
        i=M.index(np.min(M))
        mNormMinNow=M[i]
        MPCMax=(C[i+1]-C[i])/(M[i+1]-M[i])
        return mNormMinNow,MPCMax
    ...
    def solve(self): 
        ...
        self.makePFcFunc2()
        self.defValueFuncs()
        solution = ConsumerSolution(cFunc=self.cFunc, vPfunc=self.vPfunc,
...
class PerfForesightConsumerType(AgentType):
    ...
    time_inv_  = ['CRRA','Rfree','DiscFac','BoroCnstArt','aXtraCount']
    ...
    def __init__(self,cycles=0,time_flow=True,**kwds):
        ...
        aXtraCount : int
            Number of points in the grid of "assets above minimum" here the limit is bigger than the constant a
        Returns
        -------
        None
        '''       
...
def constructAssetsGrid(parameters):
    ...
    if grid_type == "linear":
        ...
        print("exception that I commented")
...
if __name__ == '__main__':
    ...
    import ConsumerParametersYG as Params
    ...
    plotFuncs(PFexample.cFunc[0],mMin,mMin+50)
